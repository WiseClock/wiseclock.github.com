...
{
    if (magnetS == null || magnetT == null || magnetS === undefined || magnetT === undefined)
        return false;
...
//#region AchievementView
joint.shapes.dialogue.AchievementView = joint.shapes.devs.ModelView.extend(
{
    template:
    [
        '<div class="node">',
        '<span class="label"></span>',
        '<button class="delete">x</button>',
        '<input type="achievement" class="title" placeholder="名称" />',
        //'<input type="text" class="name" placeholder="Text" />',
        '<p> <textarea type="text" class="name" rows="4" cols="27" placeholder="描述"></textarea></p>',
        '</div>',
    ].join(''),
    ...
    updateBox: function()
    {
        ...
        // Example of updating the HTML with a data stored in the cell model.
        var actorField = this.$box.find('input.title');
        if (!actorField.is(':focus'))
            actorField.val(this.model.get('title'));
        ...
        var label = this.$box.find('.label');
        var type = this.model.get('type').slice('dialogue.'.length);
        var typeString = type.replace('Achievement', '成就');
        label.text(typeString);
        label.attr('class', 'label ' + type);
        //this.$box.css({ width: bbox.width, height: bbox.height, left: bbox.x, top: bbox.y, transform: 'rotate(' + (this.model.get('angle') || 0) + 'deg)' });
        this.$box.css({ width: bbox.width, height: bbox.height, left: bbox.x, top: bbox.y, transform: 'rotate(' + (this.model.get('angle') || 0) + 'deg)' });
...
//#region HonorView
joint.shapes.dialogue.HonorView = joint.shapes.devs.ModelView.extend(
{
    template:
    [
        '<div class="node">',
    ...
    updateBox: function()
    {
        ...
        var label = this.$box.find('.label');
        var type = this.model.get('type').slice('dialogue.'.length);
        var typeString = type.replace('Honor', '头衔');
        label.text(typeString);
        label.attr('class', 'label ' + type);
        //this.$box.css({ width: bbox.width, height: bbox.height, left: bbox.x, top: bbox.y, transform: 'rotate(' + (this.model.get('angle') || 0) + 'deg)' });
        this.$box.css({ width: bbox.width, height: bbox.height, left: bbox.x, top: bbox.y, transform: 'rotate(' + (this.model.get('angle') || 0) + 'deg)' });
...
joint.shapes.dialogue.ChoiceView = joint.shapes.devs.ModelView.extend(
{
    ...
    [
        ...
        '<span class="label"> </span>',
        '<button class="delete">x</button>',
        '<input type="choice" class="title" placeholder="选项" />',
        '</div>',
...
//#region dialogue.Achievement
joint.shapes.dialogue.Achievement = joint.shapes.devs.Model.extend(
{
    defaults: joint.util.deepSupplement
    (
        {
...
joint.shapes.dialogue.AchievementView = joint.shapes.dialogue.AchievementView;

//#endregion

//#region dialogue.Honor
joint.shapes.dialogue.Honor = joint.shapes.devs.Model.extend(
{
    defaults: joint.util.deepSupplement
    (
        {
...
joint.shapes.dialogue.HonorView = joint.shapes.dialogue.HonorView;
